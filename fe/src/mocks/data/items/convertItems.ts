...
const readyToConvertItems = crawledItems as CrawledItems[];

const convertItems = (crawledItems: CrawledItems[]): Item[] => {
  return crawledItems.map((item) => {
    const {
      productId,
      title,
      town,
...
const saveItemsAsJson = async (crawledItems: CrawledItems[]) => {
  const items = convertItems(crawledItems);

  const data = JSON.stringify(items, null, 2); // Convert to JSON string with formatting

  const filePath = path.join(__dirname, 'convertedItems.json'); // Adjust file name if needed
